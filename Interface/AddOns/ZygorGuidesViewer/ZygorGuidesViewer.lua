...
function ZGV:FindEvent(eventName)
	...
	local cal=_G['CalendarFrame'] if not cal then Calendar_LoadUI() cal=_G['CalendarFrame'] end
	...
	for event=1, CalendarGetNumDayEvents(0,day) do --0 current month, 1 next month, -1 last month... Always want the current day.
		...
		if (texture=="CALENDAR_HARVESTFESTIVAL" and (month==9 or month==10)) then --Stay the same
		elseif (texture=="CALENDAR_HARVESTFESTIVAL" and (month==11 or month==12)) then --Lovely work around for Harvest Festival and Pilgrim's Bounty having the same texture
			texture="CALENDAR_HARVESTFESTIVAL_PILGRIM"
		end
