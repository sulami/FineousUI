...
do
	...
	if Lib then
		...
		function Lib:StepPath()  -- THE WORKHORSE.
			...
			local speeds = self.maxspeedinzone[current.m] or {1,1,1}
			...
			for no,nm in pairs(current.n) do
				...
				if no.testflags then
					...
					for flag,test in pairs(no.testflags) do
						...
						if type(test)=="string" then test=split(test,",") end
			...
			if flyspeed and not current.nofly then
				for ni,no in ipairs(self.nodes.all) do
					if current:CanFlyTo(no) then  tinsert(neighs,{no,{mode="fly"}})  end
				end
			end
		...
		function Lib:OnUpdate(elapsed)
			...
			if self.calculating and self.thread then
				...
				if num and self.is_first_run and self.PathFoundHandler then self.PathFoundHandler("progress",nil,{progress=self.calculation_step*0.02}) end
			...
			elseif event=="TAXIMAP_OPENED" then	-- highlight suggested taxi destination
				...
				if Lib.RESULTS then
					for k=1, #Lib.RESULTS do
						if Lib.RESULTS[k] and Lib.RESULTS[k].type=="taxi" and Lib.RESULTS[k].taxiFinal then
							local taxiframe = TaxiFrame
							local taxinode = Lib.RESULTS[k]
							...
							for i=1,NumTaxiNodes() do
								if TaxiNodeGetType(i)~="NONE" and TaxiNodeName(i):find(taxinode.name,0,true) then
									...
									glow:SetSize(48,48)  glow:SetPoint("CENTER",b,"CENTER",6,-5)
