...
function ZGV:Options_DefineOptionTables()
	...
	local function AddOptionGroup(groupname,groupupname,slash,groupdata,no_header)
		...
		tinsert(self.optiontables_ordered,{name=groupname,blizname="ZygorGuidesViewer"..(groupupname and ("-"..groupupname) or ""),slash=slash})
...
-- 			_default = 0.5,
		...
		AddOption('',{ type="header", name=L["opt_flash_desc"], order=13.2 })
	...
	do
		AddOption('waypoints',{
			type = 'select',
			name= "Set waypoint type",
			desc= "Choose between default waypointing or another addon.\n|cff888888Must have other addons installed to choose other options.",
			values={
	...
	do
		AddOption('arrowshow',{  type = 'toggle', set = function(i,v) Setter_Simple(i,v)  self.Pointer:UpdateArrowVisibility() end, _default=true, })
		...
		AddOption('arrowsmooth',{  type = 'toggle', width = "full", _default=true, })
	...
	do
		...
		local function setrgb(colortable,savedcolors,r,g,b,a)
			if not colortable then return end

			if r then
				colortable.r, savedcolors.r = r, r
				colortable.g, savedcolors.g = g, g
				colortable.b, savedcolors.b = b, b
				colortable.alpha, savedcolors.alpha = a, a
			else
				colortable.r = savedcolors.r
				colortable.g = savedcolors.g
				colortable.b = savedcolors.b
				colortable.alpha = savedcolors.alpha
			end
		...
		AddSubgroup('ants',{width='single'})
			AddOption('customcolorants',{ type = 'toggle', width="double"})
			...
			AddOption('singlecolorants',{ type = 'toggle', hidden = function() return not self.db.profile.customcolorants end})

			AddOption('singlecolorantscolor',{
				type = 'color',
				hidden = function()
					if self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_g, self.db.profile.singlecolorantscolor)
						setrgb(ZGV.Pointer.Icons.ant_b, self.db.profile.singlecolorantscolor)
						setrgb(ZGV.Pointer.Icons.ant_p, self.db.profile.singlecolorantscolor)
						setrgb(ZGV.Pointer.Icons.ant_y, self.db.profile.singlecolorantscolor)
						setrgb(ZGV.Pointer.Icons.ant, self.db.profile.singlecolorantscolor)
					end

					return not self.db.profile.singlecolorants or not self.db.profile.customcolorants
				end,
				get = function()  return rgb2list(self.db.profile.singlecolorantscolor)  end,
				set = function(_,r,g,b,a)
					setrgb(ZGV.Pointer.Icons.ant_g, self.db.profile.singlecolorantscolor, r, g, b, a)
					setrgb(ZGV.Pointer.Icons.ant_b, self.db.profile.singlecolorantscolor, r, g, b, a)
					setrgb(ZGV.Pointer.Icons.ant_p, self.db.profile.singlecolorantscolor, r, g, b, a)
					setrgb(ZGV.Pointer.Icons.ant_y, self.db.profile.singlecolorantscolor, r, g, b, a)
					setrgb(ZGV.Pointer.Icons.ant, self.db.profile.singlecolorantscolor, r, g, b, a)
					self:UpdateFrame()
				end,
				hasAlpha = true,
				_default={r=0.5,g=0.5,b=0.5,alpha=0.8}
			})
			...
			AddOption('colorantstaxi',{
				type = 'color',
				disabled = function()
					if not self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_g, DefaultColorG)
						--setrgb(self.db.profile.colorantstaxi, DefaultColorG)
					elseif self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_g, self.db.profile.colorantstaxi)
					end

					if not self.db.profile.customcolorants then
						self.db.profile.singlecolorants=false
						return not self.db.profile.customcolorants
					else
						if self.db.profile.customcolorants and not self.db.profile.singlecolorants then
							return not self.db.profile.customcolorants
						else
							return self.db.profile.customcolorants
						end
					end
				end,
				get = function()  return rgb2list(self.db.profile.colorantstaxi)  end,
				set = function(_,r,g,b,a) setrgb(ZGV.Pointer.Icons.ant_g, self.db.profile.colorantstaxi, r, g, b, a) self:UpdateFrame()  end,
				hasAlpha = true,
				_default={r=0.4,g=1.0,b=0.0,alpha=0.8}
			})
			...
			AddOption('colorantsship',{
				...
				disabled = function()
					if not self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_b, DefaultColorB)
						--setrgb(self.db.profile.colorantsship, DefaultColorB)
					elseif self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_b, self.db.profile.colorantsship)
					end
				...
				set = function(_,r,g,b,a)  setrgb(ZGV.Pointer.Icons.ant_b, self.db.profile.colorantsship, r, g, b, a) self:UpdateFrame()  end,
				hasAlpha = true,
				_default={r=0.0,g=0.7,b=1.0,alpha=0.8}
			})
			...
			AddOption('colorantsportal',{
				...
				disabled = function()
					if not self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_p, DefaultColorP)
						--setrgb(self.db.profile.colorantsportal, DefaultColorP)
					elseif self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_p, self.db.profile.colorantsportal)
					end
				...
				set = function(_,r,g,b,a)  setrgb(ZGV.Pointer.Icons.ant_p,self.db.profile.colorantsportal, r, g, b, a)  self:UpdateFrame()  end,
				hasAlpha = true,
				_default={r=0.8,g=0.3,b=1.0,alpha=0.8}
			})
			...
			AddOption('colorantsfly',{
				...
				disabled = function()
					if not self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_y, DefaultColorY)
						--setrgb(self.db.profile.colorantsfly, DefaultColorY)
					elseif self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant_y, self.db.profile.colorantsfly)
					end
				...
				set = function(_,r,g,b,a)  setrgb(ZGV.Pointer.Icons.ant_y, self.db.profile.colorantsfly, r, g, b, a)  self:UpdateFrame()  end,
				hasAlpha = true,
				_default = {r=1.0,g=0.8,b=0.0,alpha=0.8}
			})
			...
			AddOption('colorantsother',{--Add color to this table
				...
				disabled = function()
					if not self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant, DefaultColorO)
						--setrgb(self.db.profile.colorantsother, DefaultColorO)
					elseif self.db.profile.customcolorants and not self.db.profile.singlecolorants then
						setrgb(ZGV.Pointer.Icons.ant, self.db.profile.colorantsother)
					end
				...
				set = function(_,r,g,b,a) setrgb(ZGV.Pointer.Icons.ant, self.db.profile.colorantsother, r, g, b, a)  self:UpdateFrame()  end,
				hasAlpha = true,
				_default={r=1.0,g=1.0,b=1.0,a=0.8}
			})
			AddOptionSep()
	...
	do
		AddSubgroup('autoquest',{width='single'})
			AddOption('autoaccept',{ type = 'toggle' })
			AddOption('autoturnin',{ type = 'toggle' })
			AddOption('autoselectitem',{ type = 'toggle', hidden = function()
				if not self.db.profile.autoturnin then
					self.db.profile.autoselectitem=false
				end
				return not self.db.profile.autoturnin end})
			AddOption('fixblizzardautoaccept',{ type = 'toggle', width = "full", })
		EndSubgroup()

		AddSubgroup('item',{width='single'})
			AddOption('',{type="description",name=L['opt_item_desc']})

			AddSubgroup('loot',{width='single'})
				AddOption('autosell',{ type = 'toggle',width="double"})
				AddOption('showmoneyframe',{type = 'toggle', set = function(i,v) Setter_Simple(i,v)  MoneyFrame_SetVisible() end})
				AddOption('autobuy',{ type = 'toggle',width="double"})
				AddOption('autobuyframe',{ type='toggle', hidden=function() return not self.db.profile.autobuy end})
			EndSubgroup()

			AddSubgroup('gear',{width='single'})
				...
				AddOption('clearnotupgrades',{
					...
					func=function ()
						...
						local zgname = ZygorGuidesViewer_L("Main")["zgname"]..": Cleared the items that were not upgrades previously."
						print(zgname)
					     end,
					 width='single'
				})
...
function ZGV:Options_SetupBlizConfig()
	...
	self.optionpanels = {}
	for i,v in ipairs(self.optiontables_ordered) do
		self.optionpanels[v.name=='main' and '' or v.name] = AceConfigDialog:AddToBlizOptions(v.blizname, self.optiontables[v.name].name, v.name~='main' and self.optiontables.main.name)
	end
